...
for ($i=2; $i<=$baris; $i++)
{
		$KD_FASDIK = $data->val($i, 1);
		$NAMA_FASDIK = $data->val($i, 2);
		$ALAMAT_FASDIK	= $data->val($i, 3);
		$NAMA_KEPALA_SEKOLAH	= $data->val($i, 4);
		$KDPROP	= $data->val($i, 5);
		...
		$JKEL = $data->val($i, 17);
		$ART_KELAS = $data->val($i, 18);
		$NAMA_PENDAMPING = $data->val($i, 19);
...
?>
			...
			document.location='form_upload_pendidikan.php';
			alert(
			'<?php 
			echo "Proses Import Data Pendidikan Selesai.";
